...
fun() {  while true; do nc -l -p 8099 -e  echo -e 'HTTP/1.1 200 OK\r\nServer: EnableCM4USB\r\nDate:$(date)\r\nContent-Type: text/html; charset=UTF8\r\nCache-Control: no-store, no cache, must-revalidate\r\n\r\n<!DOCTYPE html><html><body><p>CM4 USB Enable! It is here, and there is nothing left to say.</body></html>\r\n\n\n'; done }; fun&

config='dtoverlay=uart5'
until false; do
set +e
  ...
  performWork () {
    ...
    if [ -e /tmp/$partition/config.txt ]; then
      if [ $(grep "$config" /tmp/$partition/config.txt|grep -v \#) ]; then
        echo "UART5 already enabled on $partition";
      else
